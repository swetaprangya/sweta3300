...
while True:
    ...
    def goaltrak(time,bbox):
    x,y,w,h=int(bbox[0]),int(bbox[1]),int(bbox[2]),int(bbox[3])
    #get the center point of the bonding box
    c1=x+int(w/2)
    c2=y+int(h/2)
    cv2.circle(time,(c1,c2),2,(0,255,0),3)
    cv2.circle(time,(p1,p2),2,(0,255,0),3)
    #calculate distant
    distant=math.sqrt(((c1-p1)**2)+((c2-p2)**2))
    print(distant)
    if distant<=20:
        cv2.putText(time,"goal",(300,90),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),2)
    xs.append(c1)
    ys.append(c2)    
    for i in range(len(xs)-1):
        cv2.circle(time,(xs[i],ys[i]),2,(0,255,0),2)
